...
namespace TempLat {
    ...
    public:
        ...
        void measureStandard(Model& model, T t) {
            ForLoop(i, 0, Model::Ns - 1,
                    MeansMeasurer::measure(standardOut(i), model.fldS(i), model.piS(i) * pow(model.aI, model.alpha - 2) , t); //our own contribution
                    standardOut(i).save();
                    );
        }
            ...
            ForLoop(i, 0, Model::Ns - 1,
                    if (flagON) {
                        spectraOut(i).save(t,
                            ...
                            pow(model.aI, 2 * model.alpha - 4) * PSMeasurer.powerSpectrum(model.piS(i)), //our own contribution
