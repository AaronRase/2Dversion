#ifndef COSMOINTERFACE_INITIALIZERS_GAUSSIANFLUCTUATIONSGENERATORCONST_H
#define COSMOINTERFACE_INITIALIZERS_GAUSSIANFLUCTUATIONSGENERATORCONST_H
...
namespace TempLat {
    ...
    template<typename T>
    class FluctuationsGeneratorConst {
    public:
        /* Put public methods here. These should change very little over time. */
        template <class Model>
        FluctuationsGeneratorConst(Model& model, T pLSide, std::string pSeed) :
        lSide(pLSide),
        baseSeed(pSeed)
        {
        ...
        template<class Model>
        auto getFluctuationsNorm(Model& model, Field<T> f, T mass2, T kCutOff, T Pconst) const
        {
            ...
            sayMPI << Pconst;
            return  Hcut  * (model.omegaStar / model.fStar * pow(lSide / pow<2>(f.getDx()), 1)) * pow(pow(model.fStar/model.omegaStar,2)*4*3.1415*pow(Pconst,2)/(pow(kCutOff,2)), 0.5)/sqrt(2);
        ...
        template<class Model>
        auto getNormedFluctuations(Model& model, Field<T> f, T mass2, std::string mySeed, T kCutOff, T Pconst) const
        {
            auto fFluctuationNorm =  getFluctuationsNorm(model, f,mass2, kCutOff, Pconst); //norm
        ...
        template<class Model>
        void gaussianFluctuations(Model& model, Field<T> f,  T mass2, T kCutOff, T Pconst) const
        {
            auto fLeft = getNormedFluctuations(model, f,mass2,"Random left", kCutOff, Pconst);
            // left wave
            auto fRight = getNormedFluctuations(model, f,mass2,"Random right", kCutOff, Pconst);
            // right wave
        ...
        template<class Model>
        void conjugateGaussianFluctuations(Model& model, Field<T> f, Field<T> p, T mass2, T aDot, T kCutOff, T Pconst) const
        {
            auto fLeft = getNormedFluctuations(model, f,mass2,"Random left", kCutOff, Pconst);
            auto fRight = getNormedFluctuations(model, f,mass2,"Random right", kCutOff, Pconst);
...
#ifdef TEMPLATTEST
#include "CosmoInterface/initializers/fluctuationsgeneratorconst_test.h"
#endif
